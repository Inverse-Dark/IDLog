 ...
 * @LastEditTime: 2025-12-21 13:04:08
 * @LastEditors: InverseDark
 */
#ifndef IDLOG_UTILS_LOGFACTORY_H
#define IDLOG_UTILS_LOGFACTORY_H
...
namespace IDLog
{
	...
	public:
		...
		/// @return 输出器智能指针
		virtual LogAppender::Pointer CreateLogAppender(const std::string &type, std::map<std::string, std::string> params = {},
		...
		/// @return 格式化器智能指针
		virtual Formatter::Pointer CreateFormatter(const std::string &type, std::map<std::string, std::string> params = {});

		/// @brief 创建过滤器实例
		/// @param type [IN] 过滤器类型
		/// @param params [IN] 过滤器参数键值对
		/// @return 过滤器智能指针
		virtual Filter::Pointer CreateFilter(const std::string &type, std::map<std::string, std::string> params = {});
	};
} // namespace IDLog

#endif // !IDLOG_UTILS_LOGFACTORY_H